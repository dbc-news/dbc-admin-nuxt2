...
import FormSmallButton from '@/components/form/SmallButton'
export default {
  data() {
    return {
      isLoading: true,
  ...
  methods: {
    async showArticlesNow(visible) {
      if (visible) {
        await this.getArticles('articles?per-page=20')
      }
    },
    async getArticles(endpoint) {
      try {
        this.isLoading = true
